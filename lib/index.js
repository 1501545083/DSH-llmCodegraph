...
export async function apply(ctx) {
  ...
  const S = {
    ...
    funcLogicCache: new Map(), // symbolId -> {rev, nodes, edges, note, generatedAt}
    funcLogicPromises: new Map(), // symbolId -> Promise<payload>
  }
  let seq = 0
  let pumping = false
  ...
  async function setupRoot(p) {
    ...
    S.funcLogicCache.clear()
    S.funcLogicPromises.clear()
    S.lastError = null
    cacheSaveToken++
  ...
  const FUNC_LOGIC_SYSTEM_PROMPT = 'You are a precise function-level control-flow diagram generator. You read ONE function body and always answer with exactly one valid JSON object. No markdown fences, no commentary.'
  const FUNC_LOGIC_RULES = [
    'You are given ONE function body with 1-based numbered lines. Produce the INTERNAL control-flow diagram of that function only.',
    'Return ONLY one valid JSON object, no markdown fences, no commentary.',
    'The JSON shape is: {"nodes": [{"id": "n1", "type": "start|process|decision|end", "label": "short Chinese label", "detail": "one-line explanation", "file": "relative file", "line": 12, "callee": "optional project symbol called by this node"}], "edges": [{"from": "n1", "to": "n2", "label": "branch/condition label"}]}',
    'RULES:',
    '1. Exactly one "start" node: label = function name + " 入口", file = the FILE value, line = the function start line.',
    '2. Every meaningful internal statement/step (initialization, validation, data transformation, calling another project function, state update) becomes a "process" node.',
    '3. When a process node represents a call to ANOTHER function/method of THIS project (not standard-library/framework calls), set its "callee" to the exact callee symbol: prefer the qualified form "ClassName.methodName" when visible (infer it from the object construction, e.g. "auto cm = new CncTurningDimensions(); cm->execute(...)" → "CncTurningDimensions.execute"). Use the exact name found in the source. Omit "callee" otherwise.',
    '4. Every if/else/switch/loop/guard/error-handling/early-return branch becomes a "decision" node; its outgoing edges MUST be labeled (是/否/满足/不满足/成功/失败/重试/...).',
    '5. Every return / throw / exit / terminal failure path becomes an "end" node.',
    '6. Use ONLY ids that exist in nodes. Ids must be simple strings like n1, n2, ...',
    '7. file and line MUST be copied exactly from the numbered source prefixes; file is always the FILE value.',
    '8. Max 40 nodes. Keep labels concise Chinese. Avoid trivial boilerplate steps.',
    'RETURN ONLY the JSON object.',
  ].join('\n')
  const FUNC_SOURCE_MAX = 16000
  ...
  function resolveCallee(sym, c) {
    ...
    const pool = sameFile.length ? sameFile : cands
    // 同名候选（头文件声明 vs 实现/重载）优先取函数体最长的实现，便于下钻到真实内部逻辑。
    let best = pool[0]
    for (const s of pool) {
      if ((s.endLine - s.startLine) > (best.endLine - best.startLine)) best = s
    }
    return best
  }
  ...
  // Map a logic-diagram file:line back to the tightest enclosing project symbol.
  function resolveSymbolAt(file, line) {
    if (!file || !line) return null
    const target = String(file).replace(/\\/g, '/').trim()
    if (!target) return null
    let best = null
    for (const sym of S.symbols.values()) {
      const sr = sym.rel.replace(/\\/g, '/')
      const same = sr === target || sr.endsWith('/' + target) || target.endsWith('/' + sr)
      if (!same) continue
      if (line < sym.startLine || line > sym.endLine) continue
      const kindPri = sym.kind === 'function' ? 0 : (sym.kind === 'method' || sym.kind === 'constructor' || sym.kind === 'const') ? 1 : 2
      const span = sym.endLine - sym.startLine
      if (!best || kindPri < best.kindPri || (kindPri === best.kindPri && span < best.span)) {
        best = { id: sym.id, kindPri, span }
      }
    }
    return best ? best.id : null
  }

  // Map a logic node to the project symbol it CALLS (for infinite drill-down).
  // Priority: LLM-provided "callee" field, then the enclosing symbol's extracted
  // calls at exactly this line, then calls within ±1 line.
  function nodeCalleeId(sym, n) {
    if (!sym || !n) return null
    const explicit = String(n && n.callee || '').trim().replace(/::/g, '.').replace(/\s+/g, '')
    if (explicit) {
      const r = resolveCallee(sym, { target: explicit, line: n.line || null, receiver: n.receiver != null ? String(n.receiver) : null })
      if (r) return r.id
    }
    // 只有 process 节点才做“本行调用”兜底，decision/start/end 不下钻。
    if (!n.type || n.type !== 'process') return null
    const line = int(n && n.line) || null
    if (!line) return null
    const calls = (sym.calls || []).slice().sort((a, b) => Math.abs((a.line || 0) - line) - Math.abs((b.line || 0) - line))
    for (const c of calls) {
      if (Math.abs((c.line || 0) - line) > 1) break
  ...
  function normalizeLogicDiagram(obj, rootEntries, byId) {
    ...
    rawNodes.forEach((n, i) => {
      ...
      const callee = n && n.callee != null ? String(n.callee).trim() : null
      const receiver = n && n.receiver != null ? String(n.receiver).trim() : null
      nodes.push({ id, type, label, detail, file, line, callee, receiver, entry: type === 'start' })
    ...
    // 每个逻辑节点尽量绑回项目符号：client 在最底层按 symbolId 请求函数级内部逻辑；
    // 再解析该步骤实际调用的项目符号，供“函数内部节点继续下钻”使用。
    for (const n of nodes) {
      n.symbolId = resolveSymbolAt(n.file, n.line) || null
      const encloser = n.symbolId ? S.symbols.get(n.symbolId) : null
      n.calleeId = nodeCalleeId(encloser, n) || null
    }
    return { nodes, edges }
  }

  // ================= function-level internal logic (lowest drill layer) =================
  async function buildFunctionSource(sym) {
    const f = S.filesByRel.get(sym.rel)
    if (!f) return ''
    let text = ''
    try { text = await fs.readText(f.target) } catch (err) { return '' }
    const lines = text.split('\n')
    const a = Math.max(0, (sym.startLine || 1) - 1)
    const b = Math.min(lines.length, sym.endLine || sym.startLine || 1)
    if (a >= b) return ''
    let src = ''
    for (let i = a; i < b; i++) src += (i + 1) + '|' + lines[i] + '\n'
    if (src.length > FUNC_SOURCE_MAX) src = src.slice(0, FUNC_SOURCE_MAX) + '\n...(truncated)'
    return 'FILE: ' + sym.rel + '\nFUNCTION: ' + (sym.cls ? sym.cls + '.' : '') + sym.name +
      ' [sig: ' + sym.sig + '] lines ' + sym.startLine + '-' + sym.endLine + '\n\nSOURCE:\n' + src
  }

  async function generateFunctionLogic(symId, force) {
    const sym = S.symbols.get(String(symId || ''))
    if (!sym) return null
    if (sym.kind === 'class' || sym.kind === 'module') {
      return { id: sym.id, rev: S.graphRev, nodes: [], edges: [], note: '类/模块节点不生成函数内部逻辑', generatedAt: Date.now() }
    }
    const cached = S.funcLogicCache.get(sym.id)
    if (cached && !force && cached.rev === S.graphRev) {
      // 旧版函数逻辑节点可能缺 symbolId/calleeId：就地补齐后返回。
      for (const n of cached.nodes || []) {
        if (!n.symbolId) n.symbolId = resolveSymbolAt(n.file, n.line) || sym.id
        if (!n.calleeId) n.calleeId = nodeCalleeId(sym, n) || null
      }
      return Object.assign({}, cached, { rev: S.graphRev })
    }
    if (S.funcLogicPromises.has(sym.id)) return S.funcLogicPromises.get(sym.id)
    const promise = (async () => {
      const context = await buildFunctionSource(sym)
      if (!context.trim()) {
        return { id: sym.id, rev: S.graphRev, nodes: [], edges: [], note: '函数源码不可读或行范围为空', generatedAt: Date.now() }
      }
      const obj = await llmJson(FUNC_LOGIC_RULES + '\n\n' + context, FUNC_LOGIC_SYSTEM_PROMPT)
      const rootEntry = { id: sym.id, name: sym.name, cls: sym.cls, rel: sym.rel, startLine: sym.startLine, summary: sym.summary }
      const norm = normalizeLogicDiagram(obj, [rootEntry], null)
      const payload = {
        id: sym.id,
        rev: S.graphRev,
        nodes: norm.nodes,
        edges: norm.edges,
        note: '函数内部逻辑由 LLM 生成: ' + (sym.cls ? sym.cls + '.' : '') + sym.name,
        generatedAt: Date.now(),
      }
      S.funcLogicCache.set(sym.id, payload)
      log('函数内部逻辑: ' + (sym.cls ? sym.cls + '.' : '') + sym.name + ' → ' + norm.nodes.length + ' 节点')
      return payload
    })()
    S.funcLogicPromises.set(sym.id, promise)
    try {
      return await promise
    } finally {
      S.funcLogicPromises.delete(sym.id)
    }
  }
  ...
  async function generateLogicPayload(_entryId, force) {
    ...
    if (cached && !force) {
      // 旧版缓存节点可能没有 symbolId/calleeId：返回前就地补齐，保证能无限下钻。
      for (const n of cached.nodes || []) {
        if (!n.symbolId) n.symbolId = resolveSymbolAt(n.file, n.line) || null
        if (!n.calleeId) {
          const encloser = n.symbolId ? S.symbols.get(n.symbolId) : null
          n.calleeId = nodeCalleeId(encloser, n) || null
        }
  ...
  const api = {
    ...
    'get-function-logic': async (args) => {
      const id = String(args && args.id || '')
      if (!id) return null
      return await generateFunctionLogic(id, !!(args && args.force))
    },
