...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const LLMG_CSS = `
      ...
      .llmg-tree .llmg-tree-item.llmg-internal { padding-left: 14px; }
    ...
    const NODE_GAP_X = 20
    const NODE_GAP_Y = 12
    const int = (v) => { const n = typeof v === 'number' ? Math.round(v) : parseInt(String(v), 10); return Number.isFinite(n) ? n : 0 }
    ...
    function layeredLayout(nodes, edges, W, H) {
      ...
      for (const l of layers) {
        const arr = byLayer.get(l).sort((a, b) => ((a.parentId || a.rel || a.file || '').localeCompare(b.parentId || b.rel || b.file || '')) || ((a.subLayer || 0) - (b.subLayer || 0)) || ((a.startLine || a.line || 0) - (b.startLine || b.line || 0)))
    ...
    // ---- logic progressive drill (client-side, instant after func logic arrives) ----
    // 规则：
    // 1. 未选根时只画主入口（start 节点），不展示任何下层。
    // 2. 点击入口后向下展开两层：第 1 层是业务图直接后继；第 2 层（最低层）
    //    优先拼接该后继函数经 LLM 生成的“函数级内部控制流”，无函数映射时才回退
    //    到业务图的下下层级。
    const DRILL_LEVELS = 2

    function directChildren(graph, id) {
      const byTo = new Map()
      for (const e of graph.edges || []) {
        if (e.from !== id) continue
        const n = (graph.nodes || []).find((x) => x.id === e.to)
        if (!n || byTo.has(n.id)) continue
        byTo.set(n.id, { node: n, edge: e })
      }
      return Array.from(byTo.values())
    }

    function attachFunctionLogic(outNodes, nodeMap, outEdges, parent, payload, layerBase) {
      const rawNodes = Array.isArray(payload && payload.nodes) ? payload.nodes : []
      if (!rawNodes.length) return
      const pref = 'fn:' + parent.id + '::'
      const mapped = []
      for (const n of rawNodes) {
        ...
        const copy = Object.assign({}, n, {
          id,
          layer: layerBase,
          subLayer: int(n.layer),
          entry: false,
          internal: true,
          parentId: parent.id,
          symbolId: parent.symbolId || n.symbolId || null,
        })
        mapped.push(copy)
        nodeMap.set(id, copy)
      }
      for (const copy of mapped) outNodes.push(copy)
      for (const e of payload.edges || []) {
        const from = pref + e.from, to = pref + e.to
        if (!nodeMap.has(from) || !nodeMap.has(to)) continue
        outEdges.push({ from, to, label: String(e.label || '').trim() })
      }
      const starts = mapped.filter((n) => n.type === 'start').sort((a, b) => (a.subLayer || 0) - (b.subLayer || 0))
      const target = starts[0] || mapped[0]
      if (target) outEdges.push({ from: parent.id, to: target.id, label: '函数内部' })
    }

    function addPlaceholder(outNodes, nodeMap, parent, layerBase, text) {
      const id = 'fn-load:' + parent.id
      if (nodeMap.has(id)) return
      const n = {
        id, type: 'process', label: text, detail: '等待 LLM 生成函数级内部控制流…',
        file: parent.file, line: parent.line, layer: layerBase, subLayer: 0,
        entry: false, internal: true, parentId: parent.id, symbolId: parent.symbolId || null,
      }
      outNodes.push(n)
      nodeMap.set(id, n)
    }

    function functionViewGraph(payload, symbolId) {
      const rawNodes = Array.isArray(payload && payload.nodes) ? payload.nodes : []
      if (!rawNodes.length) return null
      const pref = 'fnroot:' + symbolId + '::'
      const nodes = []
      ...
      for (const n of rawNodes) {
        ...
        const copy = Object.assign({}, n, {
          ...
          symbolId: n.symbolId || symbolId || null,
          calleeId: n.calleeId || null,
        })
        nodes.push(copy)
        nodeMap.set(id, copy)
      }
      ...
      return { nodes, edges, kind: 'function' }
    }

    function recomputeLogicGraph(graph, rootId, funcLogics, rootMeta) {
      ...
      const addProject = (n, layer) => {
        if (nodeMap.has(n.id)) return nodeMap.get(n.id)
        const copy = Object.assign({}, n, { layer, entry: !!(n.entry || n.type === 'start'), internal: false, parentId: null })
        outNodes.push(copy)
        nodeMap.set(n.id, copy)
        return copy
      }

      // 初始状态：只展示主入口（第 1 层），不做任何深度展开。
      if (!rootId) {
        for (const n of allNodes) {
          if (n.entry || n.type === 'start') addProject(n, 0)
        }
        return { nodes: outNodes, edges: outEdges, kind: 'entries' }
      }

      const root = allNodes.find((n) => n.id === rootId)
      // 函数符号作为根：直接整图展示该函数的内部控制流，内部“调用类”步骤可继续下钻。
      if (!root) {
        const func = funcLogics[rootId]
        if (func && Array.isArray(func.nodes) && func.nodes.length) {
          return functionViewGraph(func, rootId)
        }
        const label = (rootMeta && rootMeta.label) || '函数'
        const empty = !!(func && Array.isArray(func.nodes) && func.nodes.length === 0)
        const n = {
          id: 'fn-root:' + rootId,
          type: 'process',
          label: empty ? '未生成「' + label + '」函数内部逻辑' : '生成「' + label + '」函数内部逻辑…',
          detail: empty ? (func && func.note ? func.note : 'LLM 未返回内部步骤') : '等待 LLM 生成函数级内部控制流…',
          file: rootMeta ? rootMeta.file : null,
          line: rootMeta ? rootMeta.line : null,
          layer: 0, subLayer: 0,
          entry: false, internal: true, parentId: null,
          focusSymbolId: rootId, symbolId: rootId, calleeId: null,
        }
        return { nodes: [n], edges: [], kind: 'function' }
      }

      const rootCopy = addProject(root, 0)
      const children = directChildren({ nodes: allNodes, edges: allEdges }, rootId)

      if (children.length) {
        for (const child of children) {
          const c = addProject(child.node, 1)
          if (child.edge) outEdges.push({ from: rootCopy.id, to: c.id, label: String(child.edge.label || '').trim() })
          const func = c.symbolId && funcLogics[c.symbolId]
          if (func && Array.isArray(func.nodes) && func.nodes.length) {
            attachFunctionLogic(outNodes, nodeMap, outEdges, c, func, DRILL_LEVELS)
          } else {
            const grands = directChildren({ nodes: allNodes, edges: allEdges }, c.id)
            if (grands.length) {
              for (const g of grands) {
                const gc = addProject(g.node, DRILL_LEVELS)
                if (g.edge) outEdges.push({ from: c.id, to: gc.id, label: String(g.edge.label || '').trim() })
              }
            } else if (c.symbolId) {
              const empty = !!(func && Array.isArray(func.nodes) && func.nodes.length === 0)
              addPlaceholder(outNodes, nodeMap, c, DRILL_LEVELS,
                empty ? '未生成「' + (c.label || c.name) + '」函数内部逻辑' : '生成「' + (c.label || c.name) + '」函数内部逻辑…')
            }
          }
        }
      } else if (rootCopy.symbolId) {
        // 入口没有业务后继（如生命周期回调）：直接展示入口函数自身的内部逻辑。
        const func = funcLogics[rootCopy.symbolId]
        if (func && Array.isArray(func.nodes) && func.nodes.length) {
          attachFunctionLogic(outNodes, nodeMap, outEdges, rootCopy, func, 1)
        } else {
          const empty = !!(func && Array.isArray(func.nodes) && func.nodes.length === 0)
          addPlaceholder(outNodes, nodeMap, rootCopy, 1,
            empty ? '未生成「' + (rootCopy.label || rootCopy.name) + '」函数内部逻辑' : '生成「' + (rootCopy.label || rootCopy.name) + '」函数内部逻辑…')
        }
      }

      return { nodes: outNodes, edges: outEdges, kind: 'project' }
    }
    ...
    function CodeGraphView({ ctx }) {
      ...
      const [rootMeta, setRootMeta] = React.useState(null)       // {kind:'function', label/file/line/...} 或 null（项目节点根）
      const [logicHistory, setLogicHistory] = React.useState([]) // [{id, meta}] 逻辑图点击历史（可逐层返回）
      const [funcLogics, setFuncLogics] = React.useState({})     // symbolId -> 函数级内部控制流 payload
      const [funcLoading, setFuncLoading] = React.useState({})   // symbolId -> true
      const funcFetchRef = React.useRef(new Set())               // 去重：正在请求的 symbolId
      const [logicView, setLogicView] = React.useState(null)     // 当前画布实际展示的逻辑子图（入口层/两层展开）
      ...
      // 图谱或模式切换后，旧的点击历史/根节点可能失效，重置返回栈与函数级逻辑缓存
      React.useEffect(() => {
        setRootId(null)
        setRootMeta(null)
        setLogicHistory([])
        ...
        setFuncLoading({})
        setLogicView(null)
        funcFetchRef.current.clear()
      }, [graph, mode])

      // 逻辑图下钻时，为当前视图需要的项目符号请求函数级内部控制流。
      React.useEffect(() => {
        if (mode !== 'logic' || !graph || !rootId) return
        const needed = new Map()
        const projectRoot = graph.nodes.find((n) => n.id === rootId)
        if (projectRoot) {
          const children = directChildren(graph, rootId)
          for (const c of children) {
            if (c.node.symbolId) needed.set(c.node.symbolId, c.node)
          }
          if (!needed.size && projectRoot.symbolId) needed.set(projectRoot.symbolId, projectRoot)
        } else if (rootMeta && rootMeta.kind === 'function') {
          // 函数符号作为新根：加载该函数自己的内部逻辑。
          needed.set(rootId, rootMeta)
        }
        for (const [sid] of needed) {
          if (funcLogics[sid] || funcFetchRef.current.has(sid)) continue
          funcFetchRef.current.add(sid)
          setFuncLoading((prev) => Object.assign({}, prev, { [sid]: true }))
          safeCall('get-function-logic', { id: sid }).then((g) => {
            // 图谱 rev 已前进时丢弃过期响应，避免把旧函数逻辑拼到新图上。
            if (g && Array.isArray(g.nodes) && (!g.rev || !graph.rev || g.rev === graph.rev)) {
              setFuncLogics((prev) => Object.assign({}, prev, { [sid]: g }))
            }
          }).catch(() => {}).finally(() => {
            funcFetchRef.current.delete(sid)
            setFuncLoading((prev) => {
      ...
      React.useEffect(() => {
        if (!graph || !graph.nodes.length) { setLayout(null); setLogicView(null); return }
        // 项目节点根必须仍存在于 graph；函数符号根由 funcLogics 承载，不随 graph 失效。
        const isFunctionRoot = !!(rootMeta && rootMeta.kind === 'function')
        if (mode === 'logic' && rootId && !isFunctionRoot && !graph.nodes.some((n) => n.id === rootId)) {
          setRootId(null)
          setRootMeta(null)
          setLogicHistory([])
        }
        const viewGraph = mode === 'logic' ? recomputeLogicGraph(graph, rootId, funcLogics, rootMeta) : graph
        if (!viewGraph || !viewGraph.nodes.length) { setLayout(null); setLogicView(viewGraph); return }
        ...
        setLogicView(viewGraph)
        setVb(fitViewbox(nodes))
      }, [graph, mode, rootId, rootMeta, funcLogics])
      ...
      const selectLogicNode = (n) => {
        setSelected(n.id)
        setDetail({ logicNode: n })
      }
      const pushFocus = (nextId, nextMeta) => {
        if (rootId !== nextId) setLogicHistory((h) => h.concat([{ id: rootId, meta: rootMeta }]))
        setRootId(nextId)
        setRootMeta(nextMeta)
      }
      const logicNodeClick = (n) => {
        if (!n) return
        // 函数内部节点：如果它代表“调用另一个项目函数”，就把它变成新根继续向下钻；
        // 没有可钻目标时只选中查看详情。
        if (n.internal) {
          if (n.calleeId) {
            pushFocus(n.calleeId, {
              kind: 'function',
              id: n.calleeId,
              label: n.label || n.name || n.calleeId,
              file: n.file,
              line: n.line,
      ...
      const logicRootClick = (next) => {
        if (rootId !== next) setLogicHistory((h) => h.concat([{ id: rootId, meta: rootMeta }]))
        setRootId(next)
        setRootMeta(null)
        setSelected(null)
        setDetail(null)
      ...
      els.push(React.createElement('div', { className: 'llmg-toolbar', key: 'tb' },
        ...
        React.createElement('button', { className: 'llmg-tab' + (mode === 'call' ? ' on' : ''), onClick: () => { setMode('call'); setSelected(null); setDetail(null); setRootId(null); setRootMeta(null); setLogicHistory([]) } }, '调用图'),
        React.createElement('button', { className: 'llmg-tab' + (mode === 'logic' ? ' on' : ''), onClick: () => { setMode('logic'); setSelected(null); setDetail(null); setRootId(null); setRootMeta(null); setLogicHistory([]) } }, '逻辑图'),
      ...
      if (layout) {
        ...
        canvasChildren.push(React.createElement('g', { key: 'nodes' },
          layout.nodes.map((n) => {
            ...
            const internalDash = isLogic && n.internal ? '4 3' : undefined
            const shape = isDecision
              ? React.createElement('polygon', { points: (n.x) + ',' + (n.y - n.h / 2) + ' ' + (n.x + n.w / 2) + ',' + n.y + ' ' + n.x + ',' + (n.y + n.h / 2) + ' ' + (n.x - n.w / 2) + ',' + n.y, fill: color, fillOpacity: 0.16, stroke: isSel ? '#fff' : (isEntry ? '#ff6b6b' : color), strokeWidth: isSel ? 2.5 : (isEntry ? 2 : 1.2), strokeDasharray: internalDash })
              : React.createElement('rect', { x: n.x - n.w / 2, y: n.y - n.h / 2, width: n.w, height: n.h, rx: rx, fill: color, fillOpacity: 0.16, stroke: isSel ? '#fff' : (isEntry ? '#ff6b6b' : color), strokeWidth: isSel ? 2.5 : (isEntry ? 2 : 1.2), strokeDasharray: internalDash })
            return React.createElement('g', { key: n.id, opacity: dim ? 0.25 : 1, style: { cursor: 'pointer' },
              ...
              React.createElement('title', null, (n.calleeId ? '点击继续下钻到被调函数内部\n' : '') + (n.label || n.name || '')),
              shape,
      ...
      // 逻辑图右侧层级树：只反映当前画布实际展示的层级（初始仅主入口，点击后持续下钻）
      if (mode === 'logic' && logicView && logicView.nodes.length) {
        const isFunctionView = !!(rootMeta && rootMeta.kind === 'function')
        const layers = treeLayersOf(logicView)
        const treeItems = []
        ...
        layers.forEach(([l, arr]) => {
          const isLowest = !isFunctionView && rootId !== null && l === DRILL_LEVELS
          treeItems.push(React.createElement('div', { key: 'l' + l, className: 'llmg-tree-layer' },
            '第 ' + (l + 1) + ' 层（' + arr.length + ' 个）' + (isLowest ? ' · 函数内部' : '')))
          arr.forEach((n) => {
            treeItems.push(React.createElement('div', {
              key: n.id,
              className: 'llmg-tree-item' + (rootId === n.id || selected === n.id ? ' on' : '') + (n.internal ? ' llmg-internal' : ''),
              title: (n.calleeId ? '点击继续下钻 | ' : '') + (n.detail || n.file || ''),
              ...
              React.createElement('span', { className: 'dim' }, n.calleeId ? '可下钻' : (LOGIC_LABEL[n.type] || n.type)),
            ))
          })
        })
        body.push(React.createElement('div', { className: 'llmg-tree', key: 'tree' },
          React.createElement('h4', null, rootId === null ? '层级树（仅主入口）' : (isFunctionView ? '层级树（函数内部逻辑）' : '层级树（两层展开）')),
          treeItems,
        ))
      }
      ...
      if (detail) {
        ...
        if (d.logicNode) {
          ...
          rows.push(React.createElement('div', { className: 'row', key: 'n' }, React.createElement('b', null, n.label || n.name),
            ' ', React.createElement('span', { className: 'dim' }, LOGIC_LABEL[n.type] || n.type), n.internal ? ' · 函数内部' : ''))
          rows.push(React.createElement('div', { className: 'row dim', key: 'f' }, n.file, n.line ? ' : ' + n.line : ''))
          if (n.calleeId) rows.push(React.createElement('div', { className: 'row', key: 'drill' },
            React.createElement('button', { className: 'llmg-btn', onClick: () => logicNodeClick(n) }, '继续下钻：查看被调函数内部')))
      ...
      if (st) {
        ...
        if (mode === 'logic' && logicView && logicView.nodes.length) {
          const loadingCount = Object.keys(funcLoading).length
          const isFunctionView = !!(rootMeta && rootMeta.kind === 'function')
          status.push(React.createElement('span', { key: 'dr', className: 'dim' },
            !rootId ? '仅展示主入口' : (isFunctionView ? '函数内部逻辑：' + (rootMeta.label || rootId) + ' · 调用类步骤可继续下钻' : '已展开两层 · 最低层为函数内部逻辑')))
          if (loadingCount > 0) status.push(React.createElement('span', { key: 'fl', style: { color: '#e8a04c' } }, '函数内部逻辑生成中 (' + loadingCount + ')'))
        }
